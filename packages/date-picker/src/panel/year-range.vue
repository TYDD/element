...
<script type="text/babel">
  ...
  import YearRangeTable from '../basic/year-range-table';
  const calcDefaultValue = (defaultValue) => {
    if (Array.isArray(defaultValue)) {
      return [new Date(defaultValue[0]), new Date(defaultValue[1])];
    } else if (defaultValue) {
      return [new Date(defaultValue), nextTenYear(new Date(defaultValue))];
    } else {
      return [new Date(), nextTenYear(new Date())];
    }
  };
  export default {
    mixins: [Locale],
    ...
    data() {
      return {
        ...
        maxDate: '',
        leftDate: new Date(),
        rightDate: nextTenYear(new Date()),
        rangeState: {
          endDate: null,
          selecting: false,
          row: null,
    ...
    components: {ElInput, ElButton, YearRangeTable }
  };
</script>